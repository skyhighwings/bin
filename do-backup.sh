...
encrypt_dir() {
	...
	tar cJ -h --exclude=cm_socket --exclude=.dbox -C "$(dirname $dir)" "$(basename $dir)" | gpg --encrypt -r "$mykey" > "$gpg_loc";
}
...
encrypt_weechat() {
	...
	tar cJ -h --exclude=logs -C "$(dirname $dir)" "$(basename $dir)" | gpg --encrypt -r "$mykey" > "$gpg_loc";
}
